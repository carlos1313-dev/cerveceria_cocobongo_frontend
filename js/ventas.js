...
const Ventas = (() => {
  ...
  function renderRateBadge() {
    ...
    el.innerHTML = `
      ...
      <button class="rate-edit-btn" onclick="Ventas.openRateModal()" title="Actualizar manualmente">✏</button>
      <button class="rate-scrape-btn" onclick="Ventas.scrapeBCV()" title="Obtener del BCV automáticamente">🔄 BCV</button>
    `;
  }
  ...
  async function scrapeBCV() {
    const btn = document.querySelector('.rate-scrape-btn');
    UI.setLoading(btn, true);
    try {
      const response = await API.exchangeRate.scrape();
      currentRate = response.data;
      renderRateBadge();
      renderCart();
      renderCatalogList(catalog);
      UI.toast('✅ Tasa actualizada desde BCV: Bs. ' + currentRate.rate, 'success');
    } catch (err) {
      UI.toast('❌ ' + (err.message || 'Error al obtener tasa del BCV'), 'error');
    } finally {
      UI.setLoading(btn, false);
    }
  }
